package com.nt.dao;

import java.util.List;
import java.util.Set;

import org.hibernate.HibernateException;
import org.hibernate.Session;
import org.hibernate.Transaction;
import org.hibernate.query.Query;

import com.nt.entity.Programmer;
import com.nt.entity.Project;
import com.nt.utility.HibernateUtil;

public class ManyToManyDAOImpl implements ManyToManyDAO {

	@Override
	public void saveDataUsingParent() {
		Session ses=null;
		Programmer prgmr1=null,prgmr2=null,prgmr3=null;
		Project proj1=null,proj2=null;
		Transaction tx=null;
		boolean flag=false;
		// Get Session
		ses=HibernateUtil.getSession();
		//prepare objects
		prgmr1=new Programmer();
		prgmr1.setPname("raja");
		prgmr1.setSalary(90000);
		
		prgmr2=new Programmer();
		prgmr2.setPname("rajesh");
		prgmr2.setSalary(70000);
		
		prgmr3=new Programmer();
		prgmr3.setPname("karan");
		prgmr3.setSalary(50000);
		
		proj1=new Project();
		proj1.setProjName("proj1");
		proj1.setClientName("HDFC");
		
		proj2=new Project();
		proj2.setProjName("proj2");
		proj2.setClientName("ICICI");
		
		//add childs  to parent and parent to childs
		prgmr1.getProjects().add(proj1);
		prgmr1.getProjects().add(proj2);
		
		prgmr2.getProjects().add(proj2);
		
		prgmr3.getProjects().add(proj1);
		prgmr3.getProjects().add(proj2);
		
		proj1.getProgrammers().add(prgmr1);
		proj1.getProgrammers().add(prgmr3);
		proj2.getProgrammers().add(prgmr1);
		proj2.getProgrammers().add(prgmr2);
		proj2.getProgrammers().add(prgmr3);
		
		try {
			tx=ses.beginTransaction();
          		ses.save(prgmr1); ses.save(prgmr2); ses.save(prgmr3);
          		flag=true;
		}
		catch(HibernateException he) {
			he.printStackTrace();
		}
		catch(Exception e) {
			e.printStackTrace();
		}
		finally {
			if(flag) {
				tx.commit();
				System.out.println("Objects are saved");
			}
			else {
				tx.rollback();
				System.out.println("Objects are not saved");
			}
			//close session
			HibernateUtil.closeSession(ses);
		}//finally

	}//method
	
	@Override
	public void saveDataUsingChild() {
		Session ses=null;
		Programmer prgmr1=null,prgmr2=null,prgmr3=null;
		Project proj1=null,proj2=null;
		Transaction tx=null;
		boolean flag=false;
		// Get Session
		ses=HibernateUtil.getSession();
		//prepare objects
		prgmr1=new Programmer();
		prgmr1.setPname("rani");
		prgmr1.setSalary(60000);
		
		prgmr2=new Programmer();
		prgmr2.setPname("ravi");
		prgmr2.setSalary(74000);
		
		prgmr3=new Programmer();
		prgmr3.setPname("raghu");
		prgmr3.setSalary(5000);
		
		proj1=new Project();
		proj1.setProjName("proj3");
		proj1.setClientName("citiBank");
		
		proj2=new Project();
		proj2.setProjName("proj3");
		proj2.setClientName("SBI");
		
		//add childs  to parent and parent to childs
		prgmr1.getProjects().add(proj1);
		prgmr1.getProjects().add(proj2);
		
		prgmr2.getProjects().add(proj2);
		
		prgmr3.getProjects().add(proj1);
		prgmr3.getProjects().add(proj2);
		
		proj1.getProgrammers().add(prgmr1);
		proj1.getProgrammers().add(prgmr3);
		proj2.getProgrammers().add(prgmr1);
		proj2.getProgrammers().add(prgmr2);
		proj2.getProgrammers().add(prgmr3);
		
		try {
			tx=ses.beginTransaction();
          		ses.save(proj1); ses.save(proj2);
          		flag=true;
		}
		catch(HibernateException he) {
			he.printStackTrace();
		}
		catch(Exception e) {
			e.printStackTrace();
		}
		finally {
			if(flag) {
				tx.commit();
				System.out.println("Objects are saved");
			}
			else {
				tx.rollback();
				System.out.println("Objects are not saved");
			}
			//close session
			HibernateUtil.closeSession(ses);
		}//finally
	}//method
	
	@Override
	public void loadDataUsingParent() {
		Session ses=null;
		ses=HibernateUtil.getSession();
		Query query=null;
		List<Programmer> list=null;
		try {
		//prepare and execute Query
		query=ses.createQuery("from Programmer");
		list=query.list();
		
		list.stream().forEach(prgmr->{
			System.out.println("parent:"+prgmr);
			Set<Project> childs=prgmr.getProjects();
			System.out.println(childs.size());
			/*childs.stream().forEach(proj->{
				System.out.println("child::"+proj);
			});*/
		});
		}//try
		catch(HibernateException he) {
			he.printStackTrace();
		}
		catch(Exception e) {
			e.printStackTrace();
		}
		finally {
			HibernateUtil.closeSession(ses);
		}
	}//method
	
	@Override
	public void deleteAParentFromAChild() {
		Session ses=null;
		Programmer prgmr=null;
		Project proj=null;
		Set<Programmer> parents=null;
		Transaction tx=null;
		boolean flag=false;
		
		ses=HibernateUtil.getSession();
		try {
		//Load a Parent
			prgmr=ses.get(Programmer.class,3);
		//Load a Child
			proj=ses.get(Project.class, 1005);
		//get All parents of A child
			parents=proj.getProgrammers();
				tx=ses.beginTransaction();
				   parents.remove(prgmr);
				flag=true;
			}
			catch(HibernateException he){
				he.printStackTrace();
			}
			catch(Exception e) {
				e.printStackTrace();
			}
			finally {
				if(flag) {
					tx.commit();
					System.out.println("A Parent of A child got deleted ");
				}
				else {
					tx.rollback();
					System.out.println("A Parent of A child not deleted ");
				}
				//close Session
				HibernateUtil.closeSession(ses);
			}
	}//method
	
}//class
